 ...
 * Agent 核心逻辑
 *
 * 从 AgentLoop 提取的核心逻辑，支持通过回调输出各种事件
 */
...
export class AgentCore {
    ...
    async run(
        ...
        if (this.skillSystem) {
            ...
            if (skill) {
                ...
                //     callbacks.onSystemMessage(`[技能] 已激活: ${skill.name}`);
                // }
            }
        }
        ...
        if (this.context.maybeCompact()) {
            // if (callbacks.onSystemMessage) {
            //     callbacks.onSystemMessage('[系统：已压缩历史消息已节省空间]');
            // }
        }
        ...
        while (turnCount < MAX_TURNS) {
            ...
            // 4. 调用模型，流式返回内容
            const {contentBlocks, stopReason} = await this.callApi(callbacks, skillPromptAppend);
            ...
            if (stopReason === "tool_use") {
                ...
                // 8. 遍历调用所有工具
                const toolResults = await this.executeTools(toolUseBlocks, callbacks);
        ...
        if (turnCount >= MAX_TURNS) {
            if (callbacks.onSystemMessage) {
                callbacks.onSystemMessage('[警告：达到最大轮次限制]');
            }
        }
     ...
     * 调用模型，流式返回内容
     */
    private async callApi(callbacks: AgentCoreCallbacks, skillPromptAppend: string = ''): Promise<{
        ...
        for (const toolCall of currentToolCalls.values()) {
            if (toolCall.name && toolCall.id) {
                try {
                    ...
                    const toolUseBlock: InternalToolUseBlock = {
                        type: 'tool_use',
                        id: toolCall.id,
                        name: toolCall.name,
                        input,
                    ...
                    if (callbacks.onToolUse) {
                        callbacks.onToolUse(toolUseBlock);
                    }
                } catch (error) {
    ...
    private async executeTools(
        toolUseBlocks: InternalToolUseBlock[],
        callbacks: AgentCoreCallbacks
        ...
        for (let block of toolUseBlocks) {
            // if (callbacks.onSystemMessage) {
            //     callbacks.onSystemMessage(`[工具调用] ${block.name}(${JSON.stringify(block.input)})`);
            // }

            const tool = this.registry.find(block.name);
            if (!tool) {
                const errorResult: InternalToolResultBlock = {
                    type: "tool_result",
            ...
            if (decision === "deny") {
                const denyResult: InternalToolResultBlock = {
                    type: "tool_result",
                    tool_use_id: block.id,
                    content: `用户拒绝了此工具调用`,
                };
                ...
                if (callbacks.onToolResult) {
                    callbacks.onToolResult(denyResult);
                }

                if (callbacks.onSystemMessage) {
                    callbacks.onSystemMessage(`[权限] 已拒绝 ${block.name}`);
                }
                continue;
            }
            ...
            try {
                ...
                const toolResult: InternalToolResultBlock = {
                    type: "tool_result",
                    tool_use_id: block.id,
                    content: result,
                };
                ...
                // if (callbacks.onSystemMessage) {
                //     callbacks.onSystemMessage(`[工具结果] ${result.slice(0, 100)}${result.length > 100 ? "..." : ""}`);
                // }
            } catch (error) {
                ...
                if (callbacks.onSystemMessage) {
                    callbacks.onSystemMessage(`[工具异常] ${errorMsg}`);
                }
            }
        }
